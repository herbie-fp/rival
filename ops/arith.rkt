...
(define (ival-add! out x y)
  ...
  (define-values (lo-val lo-imm?) (eplinear! (ival-lo-val out) mpfr-add! xlo xlo! ylo ylo! 'down))
  (define-values (hi-val hi-imm?) (eplinear! (ival-hi-val out) mpfr-add! xhi xhi! yhi yhi! 'up))
  (ival (endpoint lo-val lo-imm?) (endpoint hi-val hi-imm?) (or xerr? yerr?) (or xerr yerr)))
...
(define (ival-sub! out x y)
  (match-define (ival (endpoint xlo xlo!) (endpoint xhi xhi!) xerr? xerr) x)
  (match-define (ival (endpoint ylo ylo!) (endpoint yhi yhi!) yerr? yerr) y)
  (define-values (lo-val lo-imm?) (eplinear! (ival-lo-val out) mpfr-sub! xlo xlo! yhi yhi! 'down))
  (define-values (hi-val hi-imm?) (eplinear! (ival-hi-val out) mpfr-sub! xhi xhi! ylo ylo! 'up))
  (ival (endpoint lo-val lo-imm?) (endpoint hi-val hi-imm?) (or xerr? yerr?) (or xerr yerr)))
...
(define (ival-hypot! out x y)
  ...
  (define-values (lo-val lo-imm?) (eplinear! (ival-lo-val out) mpfr-hypot! xlo xlo! ylo ylo! 'down))
  (define-values (hi-val hi-imm?) (eplinear! (ival-hi-val out) mpfr-hypot! xhi xhi! yhi yhi! 'up))
